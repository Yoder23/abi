...
def checklist() -> dict[str, Any]:
    return {
        ...
        "frozen_commit": "see results/abi_final_validation_v2/frozen_release_candidate_r5.json",
        "frozen_tag": "see results/abi_final_validation_v2/frozen_release_candidate_r5.json",
        "commands": [
...
def collect(root: Path, layercake_root: Path) -> list[BundleFile]:
    ...
    _append(
        records,
        root,
        "results/abi_final_validation_v2/frozen_release_candidate_r5.json",
        "repaired_frozen_candidate",
    )
    if (root / "results/abi_final_validation_v2/strict_hostile_pre_public_r5.json").is_file():
        _append(
            records,
            root,
    ...
    for relative in (
        ...
        "results/abi_final_validation_v2/live_isolation_r5_source_bound",
    ):
        _tree(
            records,
            root,
...
def build(root: Path, layercake_root: Path, output: Path) -> dict[str, Any]:
    ...
    candidate = json.loads(
        (root / "results/abi_final_validation_v2/frozen_release_candidate_r5.json").read_text(
            encoding="utf-8"
        )
    )
    manifest = {
