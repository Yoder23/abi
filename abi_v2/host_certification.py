...
CAPABILITY_SUFFIXES = {".abi", ".cake", ".abix", ".abicir"}
...
def _reject_capability_paths(paths: Iterable[str | Path]) -> None:
    supplied = [str(path) for path in paths]
    if any(Path(path).suffix.casefold() in CAPABILITY_SUFFIXES for path in supplied):
        raise HostCertificationError("capability package paths are forbidden during certification")


class _CapabilityOpenAudit:
    ...
    def __call__(self, event: str, arguments: tuple[Any, ...]) -> None:
        ...
        if isinstance(target, (str, bytes, os.PathLike)):
            ...
            if path.suffix.casefold() in CAPABILITY_SUFFIXES:
                self.blocked_attempts += 1
                raise PermissionError("capability files are unavailable during ABI V2 certification")
...
def _identity_host_checks(texts: list[str]) -> dict[str, Any]:
    for text in texts:
        if strict_utf8(text).decode("utf-8") != text:
...
def _model_host_checks(
    *, snapshot: Path, texts: list[str], forward_records: int, device: str
    ...
    with torch.inference_mode():
        for text in texts[:forward_records]:
            ...
            if torch.isfinite(logits).all().item():
                finite += 1
            argmax_hashes.append(
                hashlib.sha256(int(logits.argmax(-1).item()).to_bytes(8, "little")).hexdigest()
            )
...
def certify_host(
    ...
    *,
    ...
    device: str = "cuda",
    supplied_capability_paths: Iterable[str | Path] = (),
) -> dict[str, Any]:
    root = root.resolve()
    ...
    _reject_capability_paths(supplied_capability_paths)
    ...
    gates = {
        ...
        "capability_paths_rejected": True,
        "capability_files_unopened": audit.blocked_attempts == 0,
    ...
    result = {
        ...
        "capability_blindness": {
            "package_suffixes_denied": sorted(CAPABILITY_SUFFIXES),
            "package_paths_supplied": 0,
            "package_open_attempts": audit.blocked_attempts,
            "capability_artifact_available_to_certification_logic": False,
            "certification_completed_before_capability_reveal": True,
        },
        "gates": gates,
        "runtime": {
            "python": sys.version,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser.add_argument("--capability-package", action="append", default=[])
    ...
    result = certify_host(
        ...
        device=args.device,
        supplied_capability_paths=args.capability_package,
    )
