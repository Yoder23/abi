"""Execute frozen R15A packages in one clean R11 recipient process."""
...
def run(
    ...
    if output.exists():
        raise R14Error(f"immutable R15A recipient output exists: {output}")
    ...
    if sha256_file(before_path) != manifest["before"]["sha256"]:
        raise R14Error("R15A before package changed")
    after = []
    ...
    receipt = {
        "format": "abi-r15a-recipient-worker/1",
        "host": host_key,
        "pid": os.getpid(),
