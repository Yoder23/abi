"""Build and execute the physical R20 instruction compiler capsule."""
...
class R20IsolationError(RuntimeError):
    """Raised when physical R20 isolation cannot be established."""


def _write_json_once(path: Path, value: dict[str, Any]) -> None:
    if path.exists():
        raise R20IsolationError(f"immutable R20 output exists: {path}")
...
def _wsl_path(path: Path) -> str:
    ...
    if len(drive) != 1:
        raise R20IsolationError(f"cannot map R20 path into WSL: {resolved}")
    return f"/mnt/{drive}{resolved.as_posix().split(':', 1)[1]}"


def build_capsule(root: Path, source_bundle: Path, capsule: Path) -> dict[str, Any]:
    if capsule.exists():
        raise R20IsolationError(f"R20 capsule already exists: {capsule}")
    capsule.mkdir(parents=True)
    sources = {
        "isolated_worker.py": root / "experiments/instructional_realization_r20/isolated_worker.py",
        "source_bundle.json": source_bundle,
    }
    for name, source in sources.items():
        if not source.is_file():
            raise R20IsolationError(f"required R20 input missing: {source}")
    ...
    manifest = {
        "format": "abi-r20-isolated-instructional-capsule/1",
        "files": files,
        "prompt_rows_included": 72,
        "task_labels_included": 0,
        "expected_outputs_included": 0,
...
def run_wsl_isolated_extraction(
    ...
    if os.name != "nt" or destination.exists():
        raise R20IsolationError("registered Windows/WSL destination contract changed")
    with tempfile.TemporaryDirectory(prefix="abi-r20-stage-") as raw:
        capsule = Path(raw) / "capsule"
        manifest = build_capsule(root, source_bundle.resolve(), capsule)
        linux_capsule = f"/tmp/abi-r20-extraction-{uuid.uuid4().hex}"
        linux_runner = f"/tmp/abi-r20-runner-{uuid.uuid4().hex}.sh"
        sandbox_root = f"/tmp/abi-r20-root-{uuid.uuid4().hex}"
        command = (
            "set -euo pipefail; "
            f"cp -aL '{_wsl_path(capsule)}' '{linux_capsule}'; "
            f"cp -L '{_wsl_path(root / 'experiments/instructional_realization_r20/extraction_pivot_runner.sh')}' '{linux_runner}'; "
        ...
        if completed.returncode != 0:
            raise R20IsolationError(
                f"physical R20 extraction failed ({completed.returncode}): {completed.stderr[-4000:]}"
            )
        ...
        if not result_path.is_file():
            raise R20IsolationError("physical R20 extraction result missing")
        ...
        if (
            ...
            raise R20IsolationError("physical R20 package publication incomplete")
        launcher = {
            "format": "abi-r20-isolated-extraction-launcher/1",
