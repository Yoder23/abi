...
case "$ABI_CAPSULE_PATH" in /tmp/abi-r20-extraction-*) ;; *) exit 81 ;; esac
case "$ABI_SANDBOX_ROOT" in /tmp/abi-r20-root-*) ;; *) exit 82 ;; esac
test -f "$ABI_CAPSULE_PATH/isolated_worker.py"
test ! -e "$ABI_SANDBOX_ROOT"

mkdir -m 700 "$ABI_SANDBOX_ROOT"
mount -t tmpfs -o size=64m,mode=700 tmpfs "$ABI_SANDBOX_ROOT"
mkdir -p "$ABI_SANDBOX_ROOT"/{usr,etc,dev,proc,tmp,run,mnt,capsule,oldroot}
...
for ABI_ETC_FILE in group hostname hosts ld.so.cache localtime nsswitch.conf passwd; do
  test ! -f "/etc/$ABI_ETC_FILE" || cp -L --preserve=timestamps "/etc/$ABI_ETC_FILE" "$ABI_SANDBOX_ROOT/etc/$ABI_ETC_FILE"
done
...
for ABI_DEVICE_NAME in null zero random urandom; do
  ...
  mount --bind "/dev/$ABI_DEVICE_NAME" "$ABI_SANDBOX_ROOT/dev/$ABI_DEVICE_NAME"
done
...
test ! -d /usr/lib64 || ln -s usr/lib64 "$ABI_SANDBOX_ROOT/lib64"
...
exec chroot . /bin/bash -c '
  ...
  umount -l /oldroot
  rmdir /oldroot
  export PYTHONDONTWRITEBYTECODE=1 ABI_R20_ISOLATED=1
  cd /capsule
