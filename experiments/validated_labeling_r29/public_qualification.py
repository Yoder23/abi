...
def run(output):
    ...
    output.mkdir(parents=True); prior = root / "results/robust_labeling_r28/heldout_v1_source"; rows = [json.loads(line) for line in (prior / "source_rows.jsonl").read_text(encoding="utf-8").splitlines() if line]; extraction_rows = [row for row in rows if row["split"] == "extraction"]
    ...
    packages = [load_package(output / "extraction" / item["path"]) for item in extraction["result"]["packages"]]; facts = {fact.fact_id: fact for fact in HIDDEN_FACTS}; evaluation = []
