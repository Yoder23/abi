...
def run(output):
    ...
    output.mkdir(parents=True); prior = root / "results/validated_labeling_r29/heldout_v1_source"; rows = [json.loads(line) for line in (prior / "source_rows.jsonl").read_text(encoding="utf-8").splitlines() if line]; extraction_rows = [row for row in rows if row["split"] == "extraction"]
    ...
    result = {"format": "abi-r29-public-validated-qualification/2", "verdict": "PASS" if passed else "FAIL", "claim": "DISCLOSED_PROTOCOL_CONFORMANCE_REPAIR", "claim_ceiling": "POSTHOC_ON_V1_ROWS_NOT_FRESH_HELDOUT_OR_LAYERCAKE_IMPORT", "r29_v1_source_rows_sha256": sha256_file(prior / "source_rows.jsonl"), "metrics": metrics, "packages": extraction["result"]["packages"], "artifacts": {"evaluation": {"path": evaluation_path.name, "sha256": sha256_file(evaluation_path)}}, "full_abi_moonshot": "OPEN"}; result["evidence_sha256"] = evidence_hash(result); write_json_once(output / "result.json", result); return result
def main():
