...
def selected_facts(secret_hex, commitment, per_domain, excluded_fact_ids=()):
    ...
    for domain in sorted({fact.oracle_domain for fact in HIDDEN_FACTS}): result.extend(generator.sample([fact for fact in HIDDEN_FACTS if fact.oracle_domain == domain and fact.fact_id not in excluded], per_domain))
