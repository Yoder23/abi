...
def run(freeze_commit, config_path, reveal_path):
    root = Path(__file__).resolve().parents[2]; public = "results/validated_labeling_r29/public_v3/result.json"; host = "experiments/autonomous_labeling_r27/configs/heldout_v1.json"; prior_paths = [root / "results/validated_labeling_r29/heldout_v1_source/source_rows.jsonl", root / "results/validated_labeling_r29/heldout_v2_source/source_rows.jsonl"]
    ...
    excluded = sorted({json.loads(line)["fact_id"] for path in prior_paths for line in path.read_text(encoding="utf-8").splitlines() if line})
    config = {"format": "abi-r29-heldout-config/3", "implementation_freeze_commit": freeze_commit, "heldout_seed_commitment": hashlib.sha256(secret).hexdigest(), "reveal_sha256": sha256_file(reveal_path), "facts_per_domain": 3, "excluded_fact_ids": excluded, "label_choices_supplied": 0, "validator": "safe-stdlib-ast/1", "minimum_parse": 68, "minimum_unvalidated_raw_exact": 32, "training_authorized": False, "physical_extraction": {"distribution": "Ubuntu", "policy": "linux-pivot-root-no-network/1"}, "source": {"model_id": "Qwen/Qwen2-7B-Instruct", "revision": "f2826a00ceef68f0f2b946d945ecc0477ce4450c", "max_new_tokens": 32, "training_steps": 0}, "public_prerequisite": item(root, public), "r27_host_config": item(root, host), "code_sha256": {relative: sha256_file(root / relative) for relative in CODE_PATHS}}
