"""Score the fresh R86 catalog with the frozen R81 source method."""
...
def main() -> int:
    ...
    if (
        ...
        parser.error("R86 source input, implementation, or output state changed")
    scorer.CATALOG_SHA256 = CATALOG_SHA256
    result = scorer.run(
        Path.cwd(), args.catalog.resolve(), args.output.resolve(), args.batch_size
    )
    result["campaign"] = "r86"
    result["format"] = "abi-r86-prospective-prior-corrected-source-score/1"
    result["verdict"] = (
        "PASS_R86_SOURCE"
        if all(result.get("gates", {}).values())
        else "FAIL_R86_SOURCE"
    )
