"""Preregister the one bounded R21 label-interface repair."""
...
from .label_repair_binding import REPAIR_CODE_PATHS
...
def main() -> None:
    ...
    config = {
        "format": "abi-r21-label-repair-config/1",
        "implementation_freeze_commit": commit,
        "code_sha256": {relative: sha256_file(root / relative) for relative in REPAIR_CODE_PATHS},
        **{
        ...
        "minimum_exact_labels": 594,
        "response_regeneration_authorized": False,
        "evaluation_access_authorized": False,
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    if args.output.exists():
        raise RuntimeError("refusing to overwrite R21 label-repair config")
