"""Execute R21 after the additive, data-invariant self-hash repair."""

from __future__ import annotations

import argparse
import json
from pathlib import Path
from typing import Any, Iterable

from experiments.foreign_capability_r14.core import (
    R14Error,
    sha256_file,
    write_json_once,
)

from . import derive_labels_v3, run_v3
from .hash_assurance_binding import load_assurance_config, selfless_evidence_hash


def run(config_path: Path, source_run: Path, output: Path) -> dict[str, Any]:
    if output.exists():
        raise R14Error(f"immutable R21 assured result exists: {output}")
    root = Path(__file__).resolve().parents[2]
    config = load_assurance_config(root, config_path)
    control_config = root / str(config["label_control_config"]["path"])

    original_hash = derive_labels_v3.evidence_hash
    derive_labels_v3.evidence_hash = selfless_evidence_hash
    try:
        derived_rows = derive_labels_v3.validate_control_source(root, control_config, source_run)
        result = run_v3.run(control_config, source_run, output)
    finally:
        derive_labels_v3.evidence_hash = original_hash

    assurance = {
        "format": "abi-r21-hash-assurance-run/1",
        "assurance_config_sha256": sha256_file(config_path),
        "source_rows_revalidated_before_training": len(derived_rows),
        "result_sha256": sha256_file(output / "result.json"),
        "result_evidence_sha256": result["evidence_sha256"],
        "data_changes_performed": False,
        "gate_changes_performed": False,
        "full_abi_moonshot": "OPEN",
    }
    assurance["evidence_sha256"] = selfless_evidence_hash(assurance)
    write_json_once(output / "assurance_binding.json", assurance)
    return result


def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=Path, required=True)
    parser.add_argument("--source-run", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args(argv)
    result = run(args.config, args.source_run, args.output)
    print(json.dumps(result, indent=2))
    return 0 if result["verdict"].startswith("PASS") else 1


if __name__ == "__main__":
    raise SystemExit(main())
