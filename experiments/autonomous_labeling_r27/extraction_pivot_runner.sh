...
case "$ABI_CAPSULE_PATH" in /tmp/abi-r27-extraction-*) ;; *) exit 81 ;; esac
case "$ABI_SANDBOX_ROOT" in /tmp/abi-r27-root-*) ;; *) exit 82 ;; esac
...
ln -s usr/bin "$ABI_SANDBOX_ROOT/bin"; ln -s usr/sbin "$ABI_SANDBOX_ROOT/sbin"; ln -s usr/lib "$ABI_SANDBOX_ROOT/lib"
...
umount -l /oldroot
rmdir /oldroot
export PYTHONDONTWRITEBYTECODE=1 ABI_R27_ISOLATED=1
cd /capsule
