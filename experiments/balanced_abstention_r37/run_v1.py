"""Execute the R37 balanced abstention-only acquisition repair."""
...
def _stratum(prompt: str) -> str:
    ...
    if "python program result" in request:
        matches.append(STRATA[2])
    if len(matches) != 1:
        raise ValueError("R37 abstention row has unknown or ambiguous semantic stratum")
    return matches[0]
...
def _build(rows: list[dict[str, Any]]) -> tuple[r36.FieldAddressedTokenizer, list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    if len(schemas) != 1:
        raise RuntimeError("R37 abstention schema changed")
    ...
    eligible: dict[str, list[dict[str, Any]]] = {name: [] for name in STRATA}
    ...
    for stratum in STRATA:
        chosen = sorted(eligible[stratum], key=lambda row: (len(row["teacher_output"].encode()), row["record_id"]))[:PER_STRATUM]
        if len(chosen) != PER_STRATUM:
            raise RuntimeError(f"R37 stratum {stratum} lacks eight representable rows")
...
def run(r31_rows: Path, r31_failure: Path, r36_result: Path, output: Path) -> dict[str, Any]:
    if output.exists():
        raise RuntimeError(f"immutable R37 output exists: {output}")
    for path, digest in ((r31_rows, r34.EXPECTED_R31_ROWS_SHA256), (r31_failure, r34.EXPECTED_R31_FAILURE_SHA256), (r36_result, EXPECTED_R36_RESULT_SHA256)):
        if not path.is_file() or sha256_file(path) != digest:
            raise RuntimeError(f"R37 prerequisite missing or changed: {path}")
    prior = json.loads(r36_result.read_text(encoding="utf-8"))
    if prior["metrics"]["functional"] != 140 or prior["metrics"]["by_task"]["abstention"]["functional"] != 8:
        raise RuntimeError("R37 R36 failure diagnosis changed")
    source_rows = [row for row in base._jsonl(r31_rows) if row["split"] == "train" and row["oracle_task"] == "abstention"]
    ...
    if len(source_rows) != 48 or len(fixture) != 12:
        raise RuntimeError("R37 cardinality changed")
    tokenizer, selected, ledger = _build(source_rows)
    ...
    model, training = r34._train(api, encoded, tokenizer, 37_012)
    ...
    inherited = []
    prior_dir = r36_result.parent / "models"
    for task in [name for name in TASKS if name != "abstention"]:
        for suffix in ("safetensors", "tokenizer.json"):
            path = prior_dir / f"{task}.{suffix}"
            if not path.is_file():
                raise RuntimeError(f"R37 inherited component missing: {path}")
            inherited.append({"task": task, "path": path.relative_to(r36_result.parents[2]).as_posix(), "sha256": sha256_file(path), "bytes": path.stat().st_size})
    ...
    passed = functional == 12 and all(row["contract_exact"] for row in observations) and counts == {stratum: PER_STRATUM for stratum in STRATA} and len(inherited) == 22
    result = {
        "format": "abi-r37-balanced-abstention-development/1",
        "verdict": "PASS_BALANCED_ABSTENTION_DEVELOPMENT" if passed else "FAIL_BALANCED_ABSTENTION_DEVELOPMENT",
        "inputs": {"r31_rows_sha256": sha256_file(r31_rows), "r31_failure_sha256": sha256_file(r31_failure), "r36_result_sha256": sha256_file(r36_result), "fixture_sha256": sha256_file(fixture_path)},
        "metrics": {"evaluation_rows": len(observations), "functional": functional, "contract_exact": sum(row["contract_exact"] for row in observations), "selected_by_stratum": counts, "inherited_component_files": len(inherited)},
        ...
        "information_accounting": {"source_teacher_rows_considered": len(source_rows), "selected_teacher_outputs": len(selected), "new_teacher_calls": 0, "new_teacher_tokens": 0, "stored_logits": 0, "stored_hidden_activations": 0, "source_parameters_copied": 0, "active_parameters": training["parameters"], "elapsed_seconds": time.perf_counter() - started, "peak_gpu_memory_bytes": int(torch.cuda.max_memory_allocated()), "peak_cpu_rss_bytes": int(process.memory_info().rss)},
        ...
        "claim_ceiling": "BALANCED_ABSTENTION_DEVELOPMENT_ONLY_NOT_INTEGRATED_OR_FRESH_REPLICATION",
...
def main() -> None:
    ...
    print(json.dumps(run(args.r31_rows.resolve(), args.r31_failure.resolve(), args.r36_result.resolve(), args.output.resolve()), indent=2))
