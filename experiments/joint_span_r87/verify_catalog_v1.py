"""Fail-closed verification for the fresh R87 catalog."""
...
def main() -> int:
    ...
    if hashlib.sha256(args.catalog.read_bytes()).hexdigest() != CATALOG_SHA256:
        parser.error("R87 catalog hash changed")
    ...
    if (
        ...
        or any(
            row["probe_id"] != f"r87-reasoning-f{i // 200}-{i % 200:04d}"
            for i, row in enumerate(probes)
        )
        or any("ENT" not in row["prompt"] for row in probes)
    ):
        parser.error("R87 catalog coverage, ordering, or identity changed")
    print(json.dumps({"status": "PASS_R87_CATALOG", "rows": len(probes)}))
    return 0
