"""Build and execute the physical R17 compiler capsule."""
...
from .frames import SIGNATURES


class R17IsolationError(RuntimeError):
    """Raised when physical R17 isolation cannot be established."""


def _write_json_once(path: Path, value: dict[str, Any]) -> None:
    if path.exists():
        raise R17IsolationError(f"immutable R17 output exists: {path}")
...
def _wsl_path(path: Path) -> str:
    ...
    if len(drive) != 1:
        raise R17IsolationError(f"cannot map R17 path into WSL: {resolved}")
    return f"/mnt/{drive}{resolved.as_posix().split(':', 1)[1]}"


def build_capsule(root: Path, source_bundle: Path, capsule: Path) -> dict[str, Any]:
    if capsule.exists():
        raise R17IsolationError(f"R17 capsule already exists: {capsule}")
    capsule.mkdir(parents=True)
    sources = {
        "isolated_worker.py": root / "experiments/linguistic_realization_r17/isolated_worker.py",
        "source_bundle.json": source_bundle,
    }
    for name, source in sources.items():
        if not source.is_file():
            raise R17IsolationError(f"required R17 input missing: {source}")
        shutil.copyfile(source, capsule / name)
    spec = {
        "format": "abi-r17-generic-template-compiler/1",
        "signatures": list(SIGNATURES),
        "observations_per_signature": 3,
        "oracle_fields": 0,
        "evaluation_rows": 0,
        "success_ids": 0,
    }
    ...
    manifest = {
        "format": "abi-r17-isolated-realization-capsule/1",
        "files": files,
        "prompts_included": 0,
...
def run_wsl_isolated_extraction(
    ...
    if os.name != "nt" or destination.exists():
        raise R17IsolationError("registered Windows/WSL destination contract changed")
    with tempfile.TemporaryDirectory(prefix="abi-r17-stage-") as raw:
        capsule = Path(raw) / "capsule"
        manifest = build_capsule(root, source_bundle.resolve(), capsule)
        linux_capsule = f"/tmp/abi-r17-extraction-{uuid.uuid4().hex}"
        linux_runner = f"/tmp/abi-r17-runner-{uuid.uuid4().hex}.sh"
        sandbox_root = f"/tmp/abi-r17-root-{uuid.uuid4().hex}"
        command = (
            "set -euo pipefail; "
            f"cp -aL '{_wsl_path(capsule)}' '{linux_capsule}'; "
            f"cp -L '{_wsl_path(root / 'experiments/linguistic_realization_r17/extraction_pivot_runner.sh')}' '{linux_runner}'; "
        ...
        if completed.returncode != 0:
            raise R17IsolationError(
                f"physical R17 extraction failed ({completed.returncode}): {completed.stderr[-4000:]}"
            )
        result_path = destination / "result.json"
        if not result_path.is_file():
            raise R17IsolationError("physical R17 extraction result missing")
        result = json.loads(result_path.read_text(encoding="utf-8"))
        launcher = {
            "format": "abi-r17-isolated-extraction-launcher/1",
