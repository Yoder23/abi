"""Bind the R88 joint-span package before any candidate generation."""
...
def main() -> int:
    ...
    if args.output.exists():
        parser.error(f"immutable R88 binding exists: {args.output}")
    files = {
    ...
    if any(not path.is_file() for path in files.values()):
        parser.error("required R88 package or implementation file is absent")
    value = {
        "format": "abi-r88-joint-span-candidate-binding/1",
