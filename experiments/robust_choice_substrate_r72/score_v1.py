"""Score the frozen R72 candidate completions under pinned Phi weights."""
...
def _identity(probe_id: str) -> tuple[int, int, list[str]]:
    match = IDENTITY.fullmatch(probe_id)
    if match is None:
        raise ExtractionError(f"invalid R72 probe ID: {probe_id}")
    family, local = int(match.group(1)), int(match.group(2))
    numeric = 10_000 + family * 300 + local
    return family, local, [f"A{numeric:05d}", f"B{numeric:05d}", f"C{numeric:05d}"]


def run(root: Path, catalog_path: Path, output: Path, batch_size: int) -> dict:
    if output.exists():
        raise ExtractionError(f"immutable R72 output exists: {output}")
    if not CATALOG_SHA256 or sha256_file(catalog_path) != CATALOG_SHA256:
        raise ExtractionError("R72 scorer is unbound or catalog changed")
    if not torch.cuda.is_available() or batch_size < 1 or batch_size > 8:
        raise ExtractionError("R72 requires CUDA and batch size 1..8")
    probes = list(load_probe_catalog(catalog_path)["probes"])
    if len(probes) != ROWS or len({row["probe_id"] for row in probes}) != ROWS:
        raise ExtractionError("R72 matrix identity changed")
    ...
    if parameters != EXPECTED_PARAMETERS:
        raise ExtractionError("R72 source parameter count changed")
    ...
    for offset in range(0, len(probes), batch_size):
        ...
        for probe in group:
            ...
            family, local, choices = _identity(str(probe["probe_id"]))
            for candidate in choices:
                full_ids = tokenizer.encode(rendered + candidate, add_special_tokens=False)
                if full_ids[: len(prompt_ids)] != prompt_ids:
                    raise ExtractionError("candidate changed rendered prompt token prefix")
                candidate_ids = full_ids[len(prompt_ids):]
                if not candidate_ids:
                    raise ExtractionError("empty candidate tokenization")
                sequences.append(full_ids)
                meta.append((probe, prompt, rendered, family, local, candidate, len(prompt_ids), candidate_ids))
        ...
        for row_index, item in enumerate(meta):
            prompt_length, candidate_ids = item[6], item[7]
            positions = [starts[row_index] + prompt_length + i - 1 for i in range(len(candidate_ids))]
            ...
            if not values or any(not math.isfinite(value) for value in values):
                raise ExtractionError("non-finite candidate score")
            scored_tokens += len(values)
            scored.append({"mean_log_probability": sum(values) / len(values), "token_count": len(values)})
        for local_group, probe in enumerate(group):
            base = local_group * 3
            family, local, choices = _identity(str(probe["probe_id"]))
            scores = scored[base:base + 3]
            ...
            prompt, rendered = str(probe["prompt"]), meta[base][2]
            rows.append({
                "probe_id": probe["probe_id"], "catalog_index": offset + local_group,
                "premise_family": family, "family_local_index": local,
                ...
                "passed": selected == expected, "tie": ordered[0] == ordered[1],
                "top_margin_mean_log_probability": ordered[0] - ordered[1],
            })
        peak_rss = max(peak_rss, process.memory_info().rss)
        if len(rows) % 300 == 0 or len(rows) == ROWS:
    ...
    by_family = {
        str(family): {
            "rows": sum(row["premise_family"] == family for row in rows),
            "passing": sum(row["premise_family"] == family and row["passed"] for row in rows),
        }
        for family in range(7)
    }
    for value in by_family.values():
        value["pass_rate"] = value["passing"] / value["rows"]
    passing, ties = sum(row["passed"] for row in rows), sum(row["tie"] for row in rows)
    gates = {
        "rows": len(rows) == ROWS,
        "minimum_passing": passing >= 1_995,
        "family_floor": min(value["pass_rate"] for value in by_family.values()) >= 0.95,
        "zero_ties": ties == 0,
    }
    result = {
        "format": "abi-r72-robust-conditional-choice-weight-interrogation/1",
        "verdict": "PASS_R72_AUTHORIZE_SEGREGATED_PACKAGING" if all(gates.values()) else "FAIL_R72_ROBUST_CHOICE_SUBSTRATE",
        ...
        "metrics": {"rows": len(rows), "passing": passing, "pass_rate": passing / len(rows), "ties": ties, "by_premise_family": by_family},
        "accounting": {
            ...
            "full_vocabulary_logit_vectors_stored": 0, "hidden_activations_stored": 0,
            "source_load_seconds": load_seconds, "source_inference_seconds": inference_seconds,
