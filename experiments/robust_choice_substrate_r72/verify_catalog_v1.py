"""Fail-closed R72 catalog shape and overlap verifier."""
...
def main() -> int:
    ...
    if len(probes) != 2_100 or len({row["probe_id"] for row in probes}) != 2_100 or len({row["prompt"] for row in probes}) != 2_100 or set(families.values()) != {300} or len(families) != 7:
    ...
    result = {
        "format": "abi-r72-robust-choice-catalog-verification/1",
        ...
        "exact_historical_overlap": len(current & prior), "validation_errors": errors,
    ...
    if args.output.exists():
        raise SystemExit("R72 verification output already exists")
