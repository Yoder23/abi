...
case "$ABI_CAPSULE_PATH" in /tmp/abi-r15b-extraction-*) ;; *) exit 81 ;; esac
case "$ABI_SANDBOX_ROOT" in /tmp/abi-r15b-root-*) ;; *) exit 82 ;; esac
...
for ABI_ETC_FILE in group hostname hosts ld.so.cache localtime nsswitch.conf passwd; do
  test ! -f "/etc/$ABI_ETC_FILE" || cp -L --preserve=mode,timestamps "/etc/$ABI_ETC_FILE" "$ABI_SANDBOX_ROOT/etc/$ABI_ETC_FILE"
done
...
exec chroot . /bin/bash -c '
  ...
  umount -l /oldroot
  rmdir /oldroot
  export PYTHONDONTWRITEBYTECODE=1 ABI_R15B_ISOLATED=1
  cd /capsule
