...
See `PROTOCOL.md` and `configs/preregistered_v2.json` before running anything.
The preserved v1 draft requested more unique depth-1--3 source rows than exist;
v2 records the pre-reveal feasibility correction without changing a gate.

## Execution order

Use a new immutable revision directory. The registered first run uses
`results/native_transfer_r8/revision_001`.

```powershell
$config = "experiments/native_transfer_r8/configs/preregistered_v2.json"
$campaign = "results/native_transfer_r8/revision_001"
