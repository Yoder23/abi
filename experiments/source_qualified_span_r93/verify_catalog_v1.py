"""Verify R93 shape and exact disjointness before model access."""
...
def main() -> int:
    ...
    if CATALOG_SHA256.startswith("__") or hashlib.sha256(catalog.read_bytes()).hexdigest() != CATALOG_SHA256:
        parser.error("R93 catalog is not sealed or changed")
    ...
    for index, row in enumerate(probes):
        expected = index // 350
        if row["probe_id"] != f"r93-reasoning-f{expected}-{index % 350:04d}":
            parser.error("R93 row identity changed")
        family[str(row["structural_family"])] += 1
        placement[str(row["candidate_list_placement"])] += 1
    result = {
        "format": "abi-r93-catalog-verification/1", "verdict": "PASS_R93_CATALOG",
        "catalog_sha256": CATALOG_SHA256, "rows": len(probes),
        "unique_prompts": len(prompts), "exact_prior_prompt_overlap": len(prompts & old),
    ...
    if len(probes) != 1_400 or len(prompts) != 1_400 or prompts & old or set(family.values()) != {350} or set(placement.values()) != {350}:
        parser.error("R93 catalog shape or disjointness failed")
