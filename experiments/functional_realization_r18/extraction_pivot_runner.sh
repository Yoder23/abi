...
case "$ABI_CAPSULE_PATH" in /tmp/abi-r18-extraction-*) ;; *) exit 81 ;; esac
case "$ABI_SANDBOX_ROOT" in /tmp/abi-r18-root-*) ;; *) exit 82 ;; esac
...
exec chroot . /bin/bash -c '
  ...
  umount -l /oldroot
  rmdir /oldroot
  export PYTHONDONTWRITEBYTECODE=1 ABI_R18_ISOLATED=1
  cd /capsule
