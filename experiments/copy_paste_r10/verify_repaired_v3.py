"""Registered arithmetic and float32-codec repair for the frozen R10 verifier."""
...
ORIGINAL_COUNT_EXPRESSION = "len(expected_rows) // len(capability_ids) * len(conditions)"
REPAIRED_COUNT_EXPRESSION = "len(expected_rows) * len(conditions)"
ORIGINAL_TOLERANCE_EXPRESSION = "tolerance: float = 2e-6"
REPAIRED_TOLERANCE_EXPRESSION = "tolerance: float = 1e-5"
...
def _repaired_verify_function(root: Path) -> Any:
    ...
    for original, repaired in replacements:
        if source.count(original) != 1:
            raise R10VerifierRepairError(
                f"registered verifier expression inventory changed: {original}"
            )
        source = source.replace(original, repaired)
    namespace = {
        "__file__": str(path),
        "__name__": "experiments.copy_paste_r10._verified_repaired_v3",
...
def verify(amendment_path: Path, run_dir: Path) -> dict[str, Any]:
    ...
    if (
        amendment.get("format") != "abi-copy-paste-r10-verifier-amendment/3"
        or amendment.get("status") != "PREREGISTERED_POST_RUN_BOUNDED_VERIFIER_REPAIR"
    ...
    expected_repairs = amendment.get("exact_repairs")
    if expected_repairs != [
        {"original": ORIGINAL_COUNT_EXPRESSION, "repaired": REPAIRED_COUNT_EXPRESSION},
        {
            "original": ORIGINAL_TOLERANCE_EXPRESSION,
            "repaired": REPAIRED_TOLERANCE_EXPRESSION,
        },
    ]:
        raise R10VerifierRepairError("registered verifier repairs changed")
    ...
    result["format"] = "abi-copy-paste-r10-verification/3"
    result["verifier_amendment"] = {
        "path": amendment_path.relative_to(root).as_posix(),
        "sha256": sha256_file(amendment_path),
        "repair_scope": "per-host row arithmetic and bounded float32 comparison",
        "maximum_observed_codec_absolute_error": amendment["maximum_observed_codec_absolute_error"],
