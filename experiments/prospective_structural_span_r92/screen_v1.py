"""Prospectively screen the frozen R91 package on R92."""
...
class R92ScreenError(RuntimeError):
    pass
...
def _binding(path: Path, root: Path) -> dict[str, Any]:
    ...
    if (
        value.get("format") != "abi-r92-prospective-candidate-source-binding/1"
        or claimed != _canonical_sha(unsigned)
        or value.get("candidate_observations_before_binding") != 0
        or value.get("candidate_retrained_after_r91_development") is not False
    ):
        raise R92ScreenError("R92 binding is invalid")
    for name, item in value.get("files", {}).items():
        ...
        if not target.is_file() or target.stat().st_size != item.get("bytes") or _sha256_file(target) != item.get("sha256"):
            raise R92ScreenError(f"R92 bound file changed: {name}")
    return value


def run(*, binding_path: Path, parent: Path, artifact: Path, layercake_root: Path, output: Path) -> dict[str, Any]:
    if output.exists():
        raise R92ScreenError(f"immutable R92 screen exists: {output}")
    root = Path(__file__).resolve().parents[2]
    binding = _binding(binding_path, root)
    files = {name: root / item["path"] for name, item in binding["files"].items()}
    if (
        _sha256_file(parent / "model.safetensors") != PARENT_SHA256
        or _sha256_file(parent / "metadata.json") != PARENT_METADATA_SHA256
        or _sha256_file(artifact) != ARTIFACT_SHA256
    ):
        raise R92ScreenError("R92 parent or imported artifact changed")
    ...
    if (
        source_result.get("verdict") != "PASS_R92_SOURCE"
        or source_claim != binding.get("source_evidence_sha256")
        or source_claim != evidence_hash(unsigned_source)
        or not all(source_result.get("gates", {}).values())
    ):
        raise R92ScreenError("R92 source authorization failed")
    ...
    if len(probes) != ROWS or len(source_rows) != ROWS or set(source_rows) != {str(row["probe_id"]) for row in probes}:
        raise R92ScreenError("R92 matrix coverage changed")
    for probe in probes:
        source = source_rows[str(probe["probe_id"])]
        if (
            source["prompt_sha256"] != hashlib.sha256(str(probe["prompt"]).encode()).hexdigest()
            or source["expected_output_sha256"] != hashlib.sha256(str(probe["evaluator"]["value"]).encode()).hexdigest()
        ):
            raise R92ScreenError("R92 source row is not catalog bound")
    if not torch.cuda.is_available():
        raise R92ScreenError("R92 requires CUDA")
    ...
    torch.manual_seed(92_002)
    random_bridge = JointSpanBridge().to(device).eval()
    observations: dict[str, dict[str, Any]] = {str(row["probe_id"]): {} for row in probes}
    parent_started = time.perf_counter()
    for index, probe in enumerate(probes, 1):
        generated = _parent_generate(model, tokenizer, str(probe["prompt"]), int(probe["max_new_tokens"]), device, require_pointer=False)
        passed, score = evaluate_output(generated["output"], probe["evaluator"])
        observations[str(probe["probe_id"])]["parent"] = {**generated, "passed": bool(passed), "score": float(score)}
        if index % 200 == 0:
    ...
    for index, probe in enumerate(probes, 1):
        ...
        observations[str(probe["probe_id"])]["candidate"] = {
    ...
    for probe in probes:
        ...
        parent_row = observations[probe_id]["parent"]
        candidate = observations[probe_id]["candidate"]
        rows.append({
            "probe_id": probe_id, "premise_family": int(source["premise_family"]),
            "prompt_sha256": source["prompt_sha256"], "source_passed": bool(source["prior_corrected_passed"]),
            ...
            "source_passing_retained": bool(source["prior_corrected_passed"] and candidate["passed"]),
    ...
    metrics = {
        ...
        "random_bridge_passing": random_passing, "source_passing_retained": retained,
        ...
        "by_family": families,
        "candidate_minus_source": _bootstrap([row["candidate_passed"] for row in rows], [row["source_passed"] for row in rows], 92_001),
        "candidate_minus_parent": _bootstrap([row["candidate_passed"] for row in rows], [row["parent_passed"] for row in rows], 92_002),
        "candidate_minus_random": _bootstrap([row["candidate_passed"] for row in rows], [row["random_passed"] for row in rows], 92_003),
    ...
    gates = {
        ...
        "candidate_family_floor": min(value["candidate_passing"] for value in families.values()) >= 180,
    ...
    if not all(math.isfinite(float(value)) for value in (parent_seconds, candidate_seconds)):
        raise R92ScreenError("R92 runtime accounting is non-finite")
    passed = all(gates.values())
    result = {
        "format": "abi-r92-prospective-structural-span-screen/1",
        "verdict": "PASS_R92_BOUNDED_PROSPECTIVE_TRANSFER" if passed else "FAIL_R92_PROSPECTIVE_TRANSFER",
        ...
        "claim_boundary": "Bounded prospective two-hop structural reasoning transfer only.",
    }
