...
# -*- coding: utf-8 -*-
"""
Qwen2-1.5B follow-up variants for stricter ABI/NIB copy-paste tests.

The published exp_qwen_1p5b_nib.py PASS is preserved. This runner reuses the
same protocol but labels the Phase D trainability choice explicitly:

  ABI_D_ABI       default 256
  ABI_CAL_STEPS   default 1200
  ABI_CAL_MODE    freeze_domain_net | train_domain | freeze_all_domain

The default mode freezes the rotated domain MLP core and trains only the target
interface, domain LayerNorm, and scalar gate.
"""
...
import exp_qwen_1p5b_nib as base
...
def main():
    t_global = time.time()
    banner(f"Qwen2-1.5B ABI/NIB v2 follow-up: {TAG}")
    ...
    wiki_text, wiki_sentences, wiki_meta = base.load_wikitext_text_and_sentences(
        split="validation", min_chars=20
    )
    wiki_ids_tgt = tok_tgt(
        wiki_text, return_tensors="pt", truncation=False
    )["input_ids"].squeeze(0)[: base.MAX_WIKI]
    ...
    banner("Phase A - GPT-2-small ABI source domain training")
    t_a = time.time()
    src_model = base.GPT2SmallABI().to(base.DEVICE)
    ...
    print(f"  Phase A complete: {time.time() - t_a:.0f}s  GPT-2-small ppl={ppl_a:.1f}")

    banner("Phase C - Native Qwen2-1.5B ABI oracle")
    t_c = time.time()
    native = base.Qwen1p5BABI().to(base.DEVICE)
    ...
    print(f"  Phase C complete: {time.time() - t_c:.0f}s  Qwen native ppl={ppl_nat:.1f}")

    banner("Phase D - Procrustes + KD calibration")
    t_d = time.time()
    R = base.cross_family_procrustes(src_model, native, wiki_sentences, tok_src, tok_tgt)
    rotated_domain = base.apply_rotation_to_domain(src_model.domain, R)

    calibrated = base.Qwen1p5BABI.__new__(base.Qwen1p5BABI)
    ...
    print(f"  Phase D complete: {time.time() - t_d:.0f}s  Qwen calibrated ppl={ppl_cal:.1f}")
    ...
    results = {
        "experiment": "cross_scale_qwen_1p5b_v2",
        "name": f"exp_qwen_1p5b_nib_v2_{TAG}",
        "baseline_result": "exp_qwen_1p5b_nib_results.json",
        "variant_type": "post-baseline stricter trainability follow-up",
        ...
        "d_abi": D_ABI,
        "d_abi_note": "follow-up strict trainability variant",
        "seed": base.SEED,
        ...
        "ppl_source_gpt2_small": round(ppl_a, 3),
        ...
        "interpretation": (
            "This is a labeled follow-up to test whether the Qwen2-1.5B PASS "
            "survives a stricter copy-paste protocol with the rotated domain MLP "
            "core frozen during target calibration."
        ),
    }
    out_path = base.ROOT / f"exp_qwen_1p5b_nib_v2_{TAG}_results.json"
