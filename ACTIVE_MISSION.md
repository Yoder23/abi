...
## Current final-validation authority

The controlling release line is ABI V2 r4 content-bound validation. It is
**TECHNICALLY PASSING LOCALLY, BUT NOT YET ELIGIBLE FOR HUMAN OR INDEPENDENT
HARDWARE REVIEW**. R3 was published and reconstructed from its public manifest,
but the fresh blind Codex audit returned `FAIL`: runtime files outside the exact
capsule were not content-bound, and the review environment could not query the
exact public release endpoint. That failure is preserved rather than waived.

R4 keeps the exact allowlisted capsule and private PID/mount/network namespaces,
replaces the broad `/etc` bind with a minimal read-only runtime configuration,
and retains a canonical per-file inventory for every reachable non-virtual
entry. Every regular file is fully hashed and content-scanned; readable ZIPs
are expanded; symlinks are recorded; capability packages are detected by
internal structure, not filenames. The three host runs retain 100,511,
100,517, and 100,515 inventory rows and report zero capability signatures,
forbidden members, or campaign identifiers.

Strict raw recomputation passes with 98 bound inputs, 384 certification rows,
5,043 locked matrix rows, 3,072 genuine live causal rows, 2,100 live isolation
rows, and zero trusted scientific booleans. The disposable hostile suite
rejects 17/17 mutations—including missing inventory and a fully rehashed
missing inventory row—and restores evidence SHA-256
`7759863bb59955c396568a2e43d4545fddb2b288fe0edaa29cad9d64578136af`.

The required next order is immutable r4 commit/tag, durable publication of the
archive and four packages, public-manifest-only reconstruction in a fresh clone,
strict hostile replay in the extracted archive, and a new blind Codex audit
with live public network access. Only a passing blind audit may open the still
empty 21,000 human judgments and different-hardware execution.

The exact claim remains a bounded standalone capability-runtime across
LayerCake v25, Qwen2.5-0.5B, and Pythia-160M conformance environments. It is not
base-weight transplantation, host-model answer generation, arbitrary-model
compatibility, human validation, independent reproduction, a global minimum,
or universal superiority to LoRA/distillation.
...
quality and zero-collapse; coherence decreased to 5/100. B40 improved to
...
nearby sweep is authorized. At seed 104729, stabilized B40 failed at
1,360/1,400 with two collapses, while stabilized B80 passed at 1,383/1,400
with zero collapses and a +8.86-point paired lower-95 teacher margin. This
reproduces the intended boundary for one seed only; seeds 130363 and 155921
remain mandatory. Seed 130363 has now rejected the stabilization: B40 passes
at 1,381/1,400 with zero collapses, while B80 fails at 1,345/1,400 because
coherence is only 65/100. Only the sealed seed155921 pair remains for complete
matrix closure; the preregistered success rule can no longer be met. The final
pair is complete, leaving B40 at FAIL/PASS/FAIL and B80 at PASS/FAIL/PASS,
exactly unchanged from history. V606 rejects the uniform-exposure intervention
and closes that branch. The next bounded design may only test one deterministic
...
V657's five-fold capability-selected development bank passed every absolute
quality gate at 1,384/1,400 but retained one clarification collapse, so it was
not promotable. V661 applied the unchanged universal V2 guard across every
selected output. It preserved all 1,384 functional passes and every one of the
14 capability and critical gates while reducing collapse from one to zero.
That supports exactly one prospective physically selected, capability-isolated
sparse adaptation design; it does not promote the post-hoc bank. The design
...
The evaluator-identical V701 repair is complete on the unchanged outputs. All
243 adapted failures and all 304 inherited failures are exact-copy failures;
both routes order correctly whenever every label survives. V704 authorizes one
generic evaluator-blind prompt-span pointer feasibility diagnostic to specify
...
V177-V190 then tested a materially distinct decoder-only causal probability-
field route. Its 56,320,238-byte top-32-plus-residual artifact covers all
432,371 response positions, stores no hidden activations or source parameters,
and passed exact hostile recomputation. The generic LayerCake v6 construct
also passed independently. The 13,097,999-parameter student still failed at
853/1,400 with 57 collapses, zero coherence, and zero fact-free reasoning.
Fit attribution measured 96.88% acquisition action accuracy, 58.41% exact
acquisition sequences, 77.38% held-out teacher-forced action accuracy, and a
...
V96 preregisters that read-only attribution over all acquisition targets, all
held-out teacher targets, and all already-produced autonomous outputs. It may
load only V94, cannot train, and cannot load the teacher.

V96 passed attribution. V94 fits 95.52% of acquisition actions but only 72.85%
of held-out teacher actions, a 22.68-point gap. Its mean autonomous teacher
