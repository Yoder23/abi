"""Prospective full B50 rescreen on LayerCake's exact format-literal v22 host."""
...
FORMAT = "abi-capability-compiler-phase4-v22-b50-rescreen/1"
...
def _api(layercake_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "universal_guard_feature": UNIVERSAL_GUARD_FEATURE,
        "lexical_guard_feature": EXACT_LEXICAL_GUARD_FEATURE,
        "format_literal_feature": FORMAT_LITERAL_FEATURE,
        "format_literal_mode": FORMAT_LITERAL_MODE,
        "format_literal_declaration": FORMAT_LITERAL_DECLARATION,
        ...
        "extract_format": extract_exact_format_literals,
        "render_format": render_exact_format_literals,
    }
...
def _architecture(
    ...
    return {
        ...
        "weak_capabilities": list(api["weak_capabilities"]),
        "guard": {
            "predicate": api["guard_predicate"],
            "scope": "all_capabilities",
            "boundary": api["guard_boundary"],
...
def _package(
    ...
    manifest = api["CakeManifest"](
        schema_version="1",
        cake_id=f"abi-phase4-v22-b50-seed{spec['seed']}-english-core",
        name=f"ABI Phase 4 v22 B50 seed {spec['seed']} English core",
        description="Frozen B50 lineage on exact format-literal v22 host",
        version="0.22.0-b50-rescreen",
        ...
        evaluation_evidence={
            ...
            "status": "V22_B50_EXACT_FORMAT_DEVELOPMENT_RESCREEN",
        },
        license="Apache-2.0",
        dependencies=(),
        parent_version=None,
    ...
    if not all(gates.values()):
        raise Phase3Error(f"v22 B50 package verification failed: {gates}")
    return {
...
def _generate(
    host: Any, prompt: str, maximum: int, capability: str
    ...
    if capability in {"coherence", "format_control"}:
        ...
        return value, False, pointer, format_record
...
def run(root: Path, protocol_path: Path, output: Path) -> dict[str, Any]:
    ...
    if output.exists():
        raise Phase3Error(f"immutable v22 B50 output exists: {output}")
    ...
    for spec in protocol["systems"]:
        ...
        with tempfile.TemporaryDirectory(prefix=f"abi-v22-b50-{spec['seed']}-") as raw:
            temporary = Path(raw)
            package = _package(
            ...
            for index, probe in enumerate(probes_list):
                ...
                row = {
                    **prior,
                    ...
                    "output_changed_from_v21_history": not exact,
                }
                rows.append(row)
        ...
        pointer = {
            ...
            "evaluator_blind": all(
                row["v22_pointer"].get("evaluator_used") is False
                for row in coherence
            ),
        ...
        preservation = {
            ...
            "interface_v22_declared": protocol["interface"] == "lc-direct-neural-core/22",
        }
        machine = (
            all(quality.values())
    ...
    _write_immutable(
        aggregate_path, b"".join(canonical_json_bytes(row) for row in aggregate)
    )
    stable = all(system["machine_gates_pass"] for system in systems)
    result = {
        "format": "abi-capability-compiler-phase4-v22-b50-rescreen-result/1",
        "status": "PASS_STABLE_B50_V22_DEVELOPMENT_CANDIDATE"
        if stable
        else "FAIL_B50_V22_DEVELOPMENT_CANDIDATE",
        "protocol_sha256": protocol_sha,
        "systems": systems,
        "three_seed_all_pass": stable,
        "observations": len(aggregate),
        "model_inference_rows": len(aggregate),
        "format_transducer_rows": sum(
            system["format_executions"] for system in systems
        ),
        ...
        "claim_boundary": (
            "Three-seed B50 v22 development rescreen only. The explicit format-control "
            "path is a deterministic prompt-literal transducer and not broad neural "
            "generation. A stable pass is sufficient, not a minimum. No matched baseline, "
