...
def load_protocol(root: Path, path: Path) -> tuple[Mapping[str, Any], str]:
    ...
    if (
        ...
        or protocol.get("status") not in {"PREREGISTERED_ROUTE_BRIDGE_SCREEN", "PREREGISTERED_ROUTE_BRIDGE_EVALUATION_REPAIR"}
...
def evaluate(root:Path,path:Path,candidate:Path,output:Path)->Mapping[str,Any]:
    ...
    expected_candidate_protocol=protocol.get("candidate_protocol_sha256",protocol_hash)
    if output.exists() or metadata.get("protocol_sha256")!=expected_candidate_protocol or sha256_file(candidate/"model.safetensors")!=metadata["checkpoint"]["sha256"]:raise Phase3Error("route-bridge evaluation identity failed")
    model,tokenizer=_load_candidate(root,protocol,candidate);router_protocol=_json(root/protocol["router"]["protocol_path"]);router,router_tokenizer=sparse._load(root,router_protocol,(root/protocol["router"]["candidate_dir"]).resolve());controls=json.loads((candidate/"route_controls.json").read_text(encoding="utf-8"))
    if not isinstance(controls,list) or len(controls)!=len(CAPABILITIES):raise Phase3Error("route controls must be a complete list")
    control={x["capability"]:bytes.fromhex(x["piece_hex"]).decode() for x in controls};probes=development_probes((root/protocol["development_catalog"]).resolve());rows=[];started=time.perf_counter()
