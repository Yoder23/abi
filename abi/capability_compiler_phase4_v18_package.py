"""Package the exact successful Phase 3 composite for LayerCake v18."""
...
FORMAT = "abi-capability-compiler-phase4-v18-package/1"
...
def _load_protocol(root: Path, path: Path) -> tuple[dict[str, Any], str]:
    ...
    if (
        ...
        raise Phase3Error("v18 package governance changed")
    for relative, expected in protocol["bindings"].items():
        target = (root / relative).resolve()
        if not target.is_file() or sha256_file(target) != expected:
            raise Phase3Error(f"v18 package binding changed: {relative}")
...
def preflight(root: Path, protocol_path: Path) -> dict[str, Any]:
    ...
    if counts != expected:
        raise Phase3Error(f"v18 component parameter counts changed: {counts}")
    namespaced = {
    ...
    if namespace_counts != Counter({"model": 82, "router": 3, "residual": 4}):
        raise Phase3Error("v18 tensor namespace inventory changed")
    residual_keys = set(states["residual"])
    if residual_keys != {"down", "up", "norm.weight", "norm.bias"}:
        raise Phase3Error(f"v18 explicit residual schema changed: {sorted(residual_keys)}")
    return {
...
def build(root: Path, protocol_path: Path, output: Path) -> dict[str, Any]:
    protocol, protocol_sha = _load_protocol(root, protocol_path)
    if output.exists():
        raise Phase3Error(f"immutable v18 package output exists: {output}")
    ...
    if abi_version != protocol["interface"] or abi_sha != protocol["interface_sha256"]:
        raise Phase3Error("v18 LayerCake interface identity changed")
    states = _states(root, protocol)
    tensors = {f"{namespace}.{name}": tensor for namespace, state in states.items() for name, tensor in state.items()}
    ...
    architecture = {
        ...
        "model_tokenizer": {"format": "declarative-tokenizers-json/1", "tokenizers_json": model_tokenizer, "sha256": hashlib.sha256(model_tokenizer_raw).hexdigest(), "eos_token_id": 50256},
        "router": {"vocabulary": int(router_config["vocabulary"]), "character_hash_buckets": int(router_config["character_hash_buckets"]), "character_ngram_minimum": int(router_config["character_ngram_minimum"]), "character_ngram_maximum": int(router_config["character_ngram_maximum"]), "hash_seed": int(router_config["hash_seed"]), "classes": len(CAPABILITIES) + 1},
        ...
        "guard": {"predicate": "contiguous_1_to_16_token_span_repeated_4_times_or_fourgram_diversity_below_0.35_at_32_tokens", "scope": "weak_capabilities_only", "stop_before_collapsing_token": True, "abstention_markers": list(markers), "abstention_clause": "I cannot determine that from the information given."},
    }
    ...
    manifest = CakeManifest(
        schema_version="1", cake_id="abi-phase3-route-isolated-english-core", name="ABI route-isolated English core", description="Exact packaged Phase 3 development composite for governed conformance", version="0.18.0-dev",
        publisher={"id": "abi-research", "name": "ABI Research", "key_id": signer}, abi_version=abi_version, abi_hash=abi_sha, cake_type="portable_decoder",
        input_contract={"external": "UTF-8 bytes", "role": "english-core", "validity": "strict_utf8"}, output_contract={"external": "UTF-8 bytes", "role": "english-core", "composition": "direct_core_only_no_router", "validity": "strict_utf8"},
        architecture=architecture, supported_precisions=("fp32",), supported_backends=("pytorch", "cuda"), minimum_host_capabilities={"features": ["byte_input", "safe_tensors", "persistent_incremental_state", "physical_route_isolation", "declarative_runtime_guard", "strict_utf8_boundary"]},
        tensor_payload_hash="", tensor_shapes=tensor_specs(tensors), package_hash="", training_data_provenance={"phase1_ir_sha256": protocol["phase1_ir_sha256"], "component_sha256": {name: protocol["bindings"][relative] for name, relative in protocol["components"].items()}, "teacher_at_inference": False, "source_transformer_blocks": 0},
        evaluation_evidence={"phase3_machine_audit": protocol["phase3_certificate"], "phase3_machine_audit_sha256": protocol["bindings"][protocol["phase3_certificate"]], "status": "DEVELOPMENT_MACHINE_EVIDENCE_ONLY"}, license="Apache-2.0", dependencies=(), parent_version=None,
        signature={"algorithm": "ed25519", "key_id": signer}, domains=("english-core",), permissions=("local-inference",),
    )
    output.mkdir(parents=True)
    package_path = output / "english_core_v18.cake"
    ...
    metadata = {
        "format": "abi-capability-compiler-phase4-v18-package-result/1", "status": "PASS_EXACT_COMPOSITE_PACKAGED", "protocol_sha256": protocol_sha,
        "interface": abi_version, "interface_sha256": abi_sha,
        ...
        "tensor_namespaces": check["tensor_namespaces"], "residual_keys": check["residual_keys"], "total_parameters": check["total_parameters"],
        "source_tensor_values_changed": False, "teacher_present": False, "source_transformer_blocks": 0, "receiver_training_steps": 0, "training_performed": False, "final_test_accessed": False,
        "quality_tested": False, "phase4_certified": False, "claim_boundary": "Exact development composite packaging only; output identity, quality, information minimum, final test, Phase 4, and superiority remain unproven.",
    }
