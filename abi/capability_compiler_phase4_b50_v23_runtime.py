"""Run same-artifact CPU/GPU runtime for the exact B50 v23 package."""
...
RESULT_FORMAT = "abi-capability-compiler-phase4-b50-v23-runtime-result/1"
PROTOCOL_FORMAT = "abi-capability-compiler-phase4-b50-v23-runtime/1"
...
def _load_overlay(
    ...
    *,
    ...
    if (
        ...
        or protocol.get("status") != f"PREREGISTERED_EXACT_B50_V23_{mode.upper()}_RUNTIME"
        or protocol.get("runtime_interface") != "lc-direct-neural-core/23"
        or protocol.get("v23_runtime_mode") != mode
        or protocol.get("training_authorized") is not False
        or protocol.get("teacher_query_generation_authorized") is not False
        or protocol.get("final_test_access") != "PROHIBITED"
    ):
        raise Phase3Error("exact B50 v23 runtime governance changed")
    for relative, expected in protocol["bindings"].items():
        target = (root / relative).resolve()
        if not target.is_file() or sha256_file(target) != expected:
            raise Phase3Error(f"exact B50 v23 runtime binding changed: {relative}")
    conformance = _json(root / str(protocol["v23_conformance_result"]))
    if (
        conformance.get("status")
        != "PASS_EXACT_B50_V23_THREE_SEED_OUTPUT_CONFORMANCE"
        or not result_evidence_digest_valid(conformance)
    ):
        raise Phase3Error("exact B50 v23 conformance prerequisite changed")
    candidate = protocol["systems"]["ABI"]
    system = next(
        row
        for row in conformance["systems"]
    ...
    if (
        ...
        raise Phase3Error("exact B50 v23 runtime package identity changed")
...
def _merge(
    ...
    merged["runtime_interface"] = "lc-direct-neural-core/23"
    merged["v23_runtime_mode"] = overlay["v23_runtime_mode"]
    merged["v23_conformance_result"] = overlay["v23_conformance_result"]
    return merged


def load_cpu_protocol(root: Path, path: Path) -> tuple[dict[str, Any], str]:
    overlay, protocol_sha = _load_overlay(root, path, mode="cpu")
    base_path = root / str(overlay["base_runtime_protocol"])
    base, _ = cpu_engine.load_protocol(root, base_path)
...
def load_gpu_protocol(root: Path, path: Path) -> tuple[dict[str, Any], str]:
    ...
    base_path = root / str(overlay["base_runtime_protocol"])
    base, _ = gpu_engine.load_protocol(root, base_path)
    return _merge(base, overlay), protocol_sha


def _build_candidate_v23(
    root: Path,
    protocol: Mapping[str, Any],
    temporary: Path,
):
    source_path = root / str(protocol["candidate_screen_protocol"])
    ...
    spec = next(
        row for row in source["systems"] if int(row["seed"]) == int(expected["seed"])
    )
    ...
    if (
        ...
        raise Phase3Error("exact B50 v23 runtime rebuild changed")
    return package_path, package, public_pem, signer, api_v23, source
...
def _wrapper_result(
    ...
    *,
    ...
    result = {
        ...
        "status": (
            f"PASS_SAME_ARTIFACT_B50_V23_{mode.upper()}_RUNTIME"
            if passed
            else f"FAIL_SAME_ARTIFACT_B50_V23_{mode.upper()}_RUNTIME"
        ),
        "protocol_sha256": protocol_sha,
        "runtime_interface": "lc-direct-neural-core/23",
        "mode": mode,
        ...
        "claim_boundary": "Exact same-artifact v23 development runtime rescreen. No final test, matched-quality baseline endpoint, Phase 4, or unconditional ABI-superiority claim.",
    }
...
def preflight_cpu(root: Path, protocol_path: Path) -> dict[str, Any]:
    ...
    result["v23_protocol_sha256"] = protocol_sha
    result["runtime_interface"] = "lc-direct-neural-core/23"
    return result
...
def run_cpu(root: Path, protocol_path: Path, output: Path) -> dict[str, Any]:
    protocol, protocol_sha = load_cpu_protocol(root, protocol_path)
    if output.exists():
        raise Phase3Error(f"immutable exact B50 v23 CPU output exists: {output}")
    original = cpu_engine._build_candidate
    original_loader = cpu_engine.load_protocol
    cpu_engine._build_candidate = _build_candidate_v23
    cpu_engine.load_protocol = lambda _root, _path: (protocol, protocol_sha)
    try:
        engine = cpu_engine.run(root, protocol_path, output / "engine")
    finally:
        cpu_engine._build_candidate = original
...
def run_gpu(root: Path, protocol_path: Path, output: Path) -> dict[str, Any]:
    ...
    if output.exists():
        raise Phase3Error(f"immutable exact B50 v23 GPU output exists: {output}")
    original = gpu_engine._build_candidate
    original_loader = gpu_engine.load_protocol
    gpu_engine._build_candidate = _build_candidate_v23
    ...
    try:
        ...
        gpu_engine._build_candidate = original
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.command == "preflight-cpu":
        ...
        if not args.output_dir:
            raise Phase3Error("v23 runtime output directory is required")
