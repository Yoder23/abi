"""Hostile verifier for the native causal teacher substrate."""
...
from .capability_compiler_phase3_native_causal_extract import (
    _load_source,
    _prepare,
    _projection,
    _tensor_hash,
...
FORMAT = "abi-capability-compiler-phase3-native-causal-verifier/1"
ARTIFACT_FORMAT = "abi-capability-compiler-phase3-native-causal-teacher-substrate/1"
...
def _load_protocol(root: Path, path: Path) -> tuple[dict[str, Any], str]:
    ...
    if (
        ...
        raise Phase3Error("native causal verifier governance changed")
    for relative, expected in protocol["bindings"].items():
        target = (root / relative).resolve()
        if not target.is_file() or sha256_file(target) != expected:
            raise Phase3Error(f"native causal verifier binding changed: {relative}")
...
def _read_rows(path: Path) -> list[dict[str, Any]]:
    ...
    if any(not isinstance(row, dict) for row in rows):
        raise Phase3Error("native causal record manifest contains a non-object")
    return rows
...
def _verify_offsets(offsets: torch.Tensor, counts: Sequence[int], final: int) -> None:
    if offsets.dtype != torch.int64 or tuple(offsets.shape) != (len(counts) + 1,):
        raise Phase3Error("native causal offsets shape or dtype changed")
    expected = [0]
    for count in counts:
        if int(count) <= 0:
            raise Phase3Error("native causal record has no target values")
        expected.append(expected[-1] + int(count))
    if offsets.tolist() != expected or expected[-1] != final:
        raise Phase3Error("native causal offsets changed")


def _verify_static(
    tensors: Mapping[str, torch.Tensor], rows: Sequence[Mapping[str, Any]], expected: Mapping[str, Any]
) -> dict[str, Any]:
    if set(tensors) != {"target_values", "target_offsets"}:
        raise Phase3Error("native causal tensor keys changed")
    values = tensors["target_values"]
    if values.dtype != torch.float16 or tuple(values.shape) != (
        int(expected["target_vectors"]),
        int(expected["width"]),
    ):
        raise Phase3Error("native causal values shape or dtype changed")
    if not bool(torch.isfinite(values).all()):
        raise Phase3Error("native causal values contain nonfinite scalars")
    _verify_offsets(tensors["target_offsets"], [int(row["target_count"]) for row in rows], values.shape[0])
    norms = values.float().norm(dim=1)
    if bool((norms <= 0).any()) or float(norms.std()) <= 0:
        raise Phase3Error("native causal values are degenerate")
    return {
...
def _select(rows: Sequence[Mapping[str, Any]], seed: int, per_capability: int) -> list[int]:
    ...
    if len(grouped) != 14:
        raise Phase3Error("native causal capability inventory changed")
...
def _recompute(
    ...
    if not torch.cuda.is_available():
        raise Phase3Error("native causal exact recomputation requires CUDA")
    ...
    if _tensor_hash(projection_cpu) != protocol["projection"]["sha256"]:
        raise Phase3Error("native causal verifier projection changed")
    ...
    for start in batch_starts:
        ...
        for offset, row in enumerate(batch):
            ...
            if index not in selected_set:
                continue
            recomputed = (hidden[offset, row["predecessor_indices"]].float() @ projection).half().cpu()
            left, right = int(offsets[index]), int(offsets[index + 1])
            stored = tensors["target_values"][left:right]
            ...
            actions += stored.shape[0]
    if maximum_error != 0.0 or exact != compared:
        raise Phase3Error("native causal sampled values do not reproduce exactly")
    return {
        "sample_records": len(selected),
        "sample_actions": actions,
        "original_batches_recomputed": len(batch_starts),
        "compared_scalars": compared,
        "exact_scalars": exact,
        "exact_scalar_fraction": exact / compared,
...
def verify(root: Path, protocol_path: Path, output: Path) -> dict[str, Any]:
    ...
    if output.exists():
        raise Phase3Error("native causal verification output exists")
    artifact_dir = (root / protocol["artifact"]["directory"]).resolve()
    metadata_path = artifact_dir / "metadata.json"
    metadata = _json(metadata_path)
    if (
        metadata.get("format") != ARTIFACT_FORMAT
        or metadata.get("status") != "EXTRACTED_UNVERIFIED_TRAINING_PROHIBITED"
        or metadata.get("evidence_sha256") != _evidence_hash(metadata)
    ):
        raise Phase3Error("native causal metadata changed")
    tensor_path = artifact_dir / metadata["artifact"]["path"]
    records_path = artifact_dir / metadata["records"]["path"]
    for path, expected in (
        (metadata_path, protocol["artifact"]["metadata_sha256"]),
        (tensor_path, protocol["artifact"]["tensor_sha256"]),
        (records_path, protocol["artifact"]["records_sha256"]),
    ):
        if not path.is_file() or sha256_file(path) != expected:
            raise Phase3Error(f"native causal artifact changed: {path.name}")
    rows = _read_rows(records_path)
    phase1 = load_phase1_ir((root / protocol["phase1_ir"]).resolve())
    if len(rows) != int(protocol["expected"]["records"]) or len(phase1) != len(rows):
        raise Phase3Error("native causal record count changed")
    capabilities = Counter()
    for source, row in zip(phase1, rows):
        if (
            row["record_id"] != source["ir_record_id"]
            or row["capability"] != source["capability"]
            or row["prompt_sha256"] != source["normalized_acquisition_prompt_sha256"]
            or row["output_sha256"] != source["normalized_output_sha256"]
            or int(row["target_count"]) != len(source["normalized_output_token_ids"])
        ):
            raise Phase3Error("native causal provenance join changed")
        capabilities[row["capability"]] += 1
    if set(capabilities.values()) != {500} or len(capabilities) != 14:
        raise Phase3Error("native causal capability balance changed")
    ...
    result = {
        "format": "abi-capability-compiler-phase3-native-causal-verification-result/1",
        "status": "PASS_ARTIFACT_VERIFIED_TRAINING_PROTOCOL_DESIGN_AUTHORIZED",
        "protocol_sha256": protocol_sha,
        "artifact": {"tensor_sha256": sha256_file(tensor_path), "records_sha256": sha256_file(records_path), "metadata_sha256": sha256_file(metadata_path)},
        ...
        "claim_boundary": "This verifies only the native causal teacher substrate; learned transfer, autonomous quality, speed, and Phase 3 remain unproven.",
    }
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser.add_argument("--protocol", default="ABI_CAPABILITY_COMPILER_PHASE3_NATIVE_CAUSAL_VERIFIER_PROTOCOL_V92.json")
    parser.add_argument("--output", default="results/abi_capability_compiler_phase3_native_causal/verification_v92.json")
