"""Read-only route-diagonal residual capacity screen at replacement layer 1."""
...
FORMAT = "abi-capability-compiler-phase3-route-diagonal-span-oracle/1"


def _project(values: torch.Tensor, basis: torch.Tensor) -> torch.Tensor:
    return (values @ basis) @ basis.transpose(0, 1)
...
def execute(root: Path, protocol_path: Path, output: Path) -> dict:
    ...
    if (
        ...
        or protocol.get("status") != "PREREGISTERED_READ_ONLY_ROUTE_DIAGONAL_SPAN_ORACLE"
        or protocol.get("training_authorized") is not False
        or protocol.get("artifact_write") != "PROHIBITED"
        or protocol.get("final_test_access") != "PROHIBITED"
        or protocol.get("sweeps_authorized") is not False
    ):
        raise Phase3Error("route-diagonal span governance changed")
    for name, expected in protocol["bindings"].items():
        path = Path(name) if Path(name).is_absolute() else root / name
        if not path.is_file() or sha256_file(path) != expected:
            raise Phase3Error(f"route-diagonal span binding changed: {name}")
    if output.exists() or not torch.cuda.is_available():
        raise Phase3Error("route-diagonal output exists or CUDA unavailable")
    ...
    def capture(rows: list[dict], *, validation: bool) -> list[dict]:
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            for index, row in enumerate(rows):
                ...
                expected_route = routed._route(str(row["capability"]))
                route_exact += int(route_index == expected_route)
                ...
                residual = native - attention
                item = {
                    "record_id": row["record_id"],
                    "capability": row["capability"],
                    "route": route_index,
                    "feature": feature.squeeze(0).float().cpu(),
                    "residual": residual.squeeze(0).float().cpu(),
                }
                if validation:
    ...
    baseline_rank_energy = float(eigenvalues[:rank].sum() / eigenvalues.sum().clamp_min(1e-12))

    routes = len(protocol["route_names"])
    numerators = torch.zeros(routes, width, dtype=torch.float64, device=device)
    denominators = torch.zeros_like(numerators)
    route_observations = [0] * routes
    for row in train_cache:
        feature = row["feature"].to(device)
        residual = row["residual"].to(device)
        centered = residual - mean
        complement = centered - _project(centered, basis)
        route_index = int(row["route"])
        numerators[route_index].add_((feature * complement).sum(dim=0).double())
        denominators[route_index].add_(feature.square().sum(dim=0).double())
        route_observations[route_index] += feature.shape[0]
    diagonal = (numerators / denominators.clamp_min(1e-20)).float()
    ...
    for row in train_cache:
        feature = row["feature"].to(device)
        residual = row["residual"].to(device)
        diagonal_delta = feature * diagonal[int(row["route"])]
        remainder = residual - mean - diagonal_delta
        reconstruction = mean + diagonal_delta + _project(remainder, basis)
        centered_sum_squares += float((residual - mean).square().sum())
        reconstruction_sum_squares += float((residual - reconstruction).square().sum())
    adjusted_train_capture = 1.0 - reconstruction_sum_squares / max(centered_sum_squares, 1e-12)
    ...
    with torch.inference_mode():
        for row in validation_cache:
            ...
            diagonal_delta = feature * diagonal[int(row["route"])]
            remainder = residual - mean - diagonal_delta
            predicted_residual = mean + diagonal_delta + _project(remainder, basis)
    ...
    gates = {
        "adjusted_train_residual_capture": adjusted_train_capture
        >= float(protocol["gates"]["adjusted_train_residual_capture_minimum"]),
    ...
    result = {
        "format": FORMAT,
        "status": "PASS_ROUTE_DIAGONAL_SPAN_ORACLE" if passed else "FAIL_ROUTE_DIAGONAL_SPAN_ORACLE",
        "protocol_sha256": sha256_file(protocol_path),
        "rank": rank,
        "baseline_rank_energy": baseline_rank_energy,
        "adjusted_train_residual_capture": adjusted_train_capture,
        "train_observations": observations,
        "route_observations": route_observations,
        "route_diagonal_l2_norms": [float(torch.linalg.vector_norm(row)) for row in diagonal],
        "validation": {
        ...
        "stored_parameters_per_layer": routes * width,
        "active_multiply_adds_per_token": width,
        "training_performed": False,
        "artifact_written": False,
        "final_test_accessed": False,
        "phase3_certified": False,
        "claim_boundary": "Read-only route-diagonal plus direct-rank-coefficient layer-one capacity oracle only; no realizable coefficient model, checkpoint, autonomous quality, runtime, Phase 3, or superiority claim.",
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--protocol",
        default="ABI_CAPABILITY_COMPILER_PHASE3_ROUTE_DIAGONAL_SPAN_ORACLE_PROTOCOL_V395.json",
    )
    parser.add_argument(
        "--output-dir",
        default="results/abi_capability_compiler_phase3_native_trajectory/route_diagonal_span_v396",
    )
