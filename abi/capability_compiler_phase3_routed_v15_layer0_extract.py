"""Fail-fast routed v15 layer-0 extraction on the exact replacement interface."""
...
FORMAT = "abi-capability-compiler-phase3-routed-v15-layer0-extract/1"
ROUTES = ("generic", "abstention", "conversation")


def _route(capability: str) -> int:
    return ROUTES.index(capability) if capability in ROUTES[1:] else 0


def _attention(layer, hidden: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
    normalized = layer.input_norm(hidden)
    primary_latent = layer.attention_input_projection(normalized)
    query, key, value = layer._qkv(primary_latent, positions)
    attended = layer._attention(query, key, value, causal=True).transpose(1, 2).contiguous().view_as(primary_latent)
    secondary_latent = layer.secondary_attention_input_projection(normalized)
    query2, key2, value2 = layer._secondary_qkv(secondary_latent, positions)
    attended2 = layer._attention(query2, key2, value2, causal=True).transpose(1, 2).contiguous().view_as(secondary_latent)
    return (
        hidden
        + layer.attention_output_projection(layer.o_proj(attended))
        + layer.secondary_attention_output_projection(layer.secondary_o_proj(attended2))
    )
...
def execute(root: Path, protocol_path: Path, output: Path) -> dict:
    ...
    if (
        ...
        or protocol.get("status") != "PREREGISTERED_FAIL_FAST_ROUTED_V15_LAYER0_EXTRACTION"
        or protocol.get("device") != "cuda"
        or protocol.get("final_test_access") != "PROHIBITED"
    ):
        raise Phase3Error("routed v15 layer0 governance changed")
    for name, expected in protocol["bindings"].items():
        target = Path(name) if Path(name).is_absolute() else root / name
        if not target.is_file() or sha256_file(target) != expected:
            raise Phase3Error(f"routed v15 layer0 binding changed: {name}")
    ...
    router_checkpoint = load_file(str(root / protocol["router_checkpoint"]["path"]), device="cpu")
    with torch.no_grad():
        model.router.weight.copy_(router_checkpoint["router.weight"].to(model.router.weight.dtype))
        model.router.bias.copy_(router_checkpoint["router.bias"].to(model.router.bias.dtype))
    ...
    with torch.no_grad():
        for name, value in primary_checkpoint.items():
            if name.startswith("layers.0.") and name in state and state[name].shape == value.shape and not name.startswith("layers.0.mlp_"):
    ...
    source_layer = teacher.model.layers[0]
    ...
    attention_rmses = []; attention_cosines = []
    with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):
        for row in train_rows:
            ids = torch.tensor([row["input_ids"]], dtype=torch.long, device=device)
            hidden = model.token_embedding(ids)
            teacher_attention, teacher_final = dual._teacher_components(teacher, 0, hidden)
            attention = _attention(layer, hidden, torch.arange(ids.shape[1], device=device))
            ...
            token_routes.extend([_route(str(row["capability"]))] * feature_cpu.shape[0])
    ...
    rmses = []; cosines = []; route_exact = 0; records = []
    with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):
        for row in validation_rows:
            ...
            source_ids = torch.tensor([example_by_id[str(row["record_id"])]["source_ids"]], dtype=torch.long, device=device)
            predicted_route = model._select_route(source_ids); expected_route = _route(str(row["capability"]))
            route_exact += int(predicted_route == expected_route)
            prediction = attention + layer._mlp_delta(attention, predicted_route)
            rmse, cosine = dual.base._metrics(prediction, teacher_final, hidden)
            rmses.append(float(rmse)); cosines.append(float(cosine))
            records.append({"record_id": row["record_id"], "capability": row["capability"], "expected_route": ROUTES[expected_route], "predicted_route": ROUTES[predicted_route], "relative_rmse": float(rmse), "output_cosine": float(cosine)})
    ...
    passed = (
        ...
        and route_exact == len(validation_rows)
    )
    checkpoint = {
        name: parameter.detach().to(torch.float16).cpu().contiguous()
        for name, parameter in model.named_parameters()
        if name.startswith("layers.0.") or name.startswith("router.")
    }
    checkpoint_path = output / "routed_v15_layer_00.safetensors"
    save_file(checkpoint, str(checkpoint_path), metadata={"format": FORMAT, "protocol_sha256": sha256_file(protocol_path)})
    result = {
        "format": FORMAT,
        "status": "PASS_ROUTED_V15_LAYER0" if passed else "FAIL_ROUTED_V15_LAYER0",
        "protocol_sha256": sha256_file(protocol_path),
        "layer": 0,
        "copied_primary_attention_tensor_keys": len(copied_primary),
        "secondary_attention_exactly_zero": all(
            torch.count_nonzero(parameter).item() == 0
            for name, parameter in layer.named_parameters() if name.startswith("secondary_")
        ),
        "calibration_tokens": calibration_tokens,
        "train_observations": observations,
        "route_train_observations": dict(zip(ROUTES, route_observations)),
        "basis_rank": rank,
        ...
        "claim_boundary": "Fail-fast routed v15 layer-0 extraction only; no full artifact, English quality, physical runtime, certificate, or superiority claim.",
    }
...
def main() -> int:
    ...
    parser.add_argument("--protocol", default="ABI_CAPABILITY_COMPILER_PHASE3_ROUTED_V15_LAYER0_EXTRACT_PROTOCOL_V305.json")
    parser.add_argument("--output", default="results/abi_capability_compiler_phase3_routed_v15/layer0_v306")
