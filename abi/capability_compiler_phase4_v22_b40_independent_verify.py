"""Read-only independent verifier for the complete v22 B40 boundary screen."""
...
from .capability_compiler_phase4_v22_b40_screen import (
    _package,
    load_protocol as load_source_protocol,
)
from .capability_compiler_phase4_v22_b50_independent_verify import format_semantics
from .capability_compiler_phase4_v22_b50_rescreen import _api


FORMAT = "abi-capability-compiler-phase4-v22-b40-independent-verify/1"
...
def independent_b40_preservation(
    ...
    return {
        ...
        "nonformat_change_set_equals_historical_collapses": {
            str(row["probe_id"]) for row in changed
        }
        == {str(row["probe_id"]) for row in historical_collapses},
        ...
        "changed_nonformat_guard_terminated": all(
            bool(row["guard_terminated"]) for row in changed
        ),
        "changed_nonformat_canonical_prefix": all(
            bool(row["canonical_historical_prefix_preserved"]) for row in changed
        ),
...
def load_protocol(root: Path, path: Path) -> tuple[dict[str, Any], str]:
    ...
    if (
        ...
        or protocol.get("status") != "PREREGISTERED_READ_ONLY_V22_B40_INDEPENDENT_VERIFIER"
        ...
        raise Phase3Error("v22 B40 independent-verifier governance changed")
    for relative, expected in protocol["bindings"].items():
        target = (root / relative).resolve()
        if not target.is_file() or sha256_file(target) != expected:
            raise Phase3Error(f"v22 B40 independent-verifier binding changed: {relative}")
...
def run(root: Path, protocol_path: Path, output: Path) -> dict[str, Any]:
    ...
    if output.exists():
        raise Phase3Error(f"immutable v22 B40 verifier output exists: {output}")
    ...
    for system in recorded["systems"]:
        ...
        for row in rows:
            ...
            checks = {
                ...
                "historical_collapse": bool(row["historical_repetition_collapse_v2"])
        ...
        quality, relative = _quality_gates(
            source, evaluation, rows, probes, teacher, seed + 7_100_000
        )
        ...
        preservation = independent_b40_preservation(rows, weak, source["interface"])
        with tempfile.TemporaryDirectory(prefix=f"abi-v22-b40-verify-{seed}-") as raw:
            rebuilt = _package(
                root,
                source,
                spec,
        ...
        gates = {
            ...
            "teacher_comparison_recomputed": relative == system["teacher_comparison_v1"],
            ...
            "changed_nonformat_count_recomputed": len(changed_nonformat)
            == system["changed_nonformat_rows"],
        ...
        verified.append(
            {
                ...
                "clarification_passes_v1": evaluation["per_capability"]["clarification"][
                    "passes_v1"
                ],
                "clarification_wilson_lower_95": evaluation["per_capability"][
                    "clarification"
                ]["wilson_v1"]["lower_95"],
    ...
    topology = [system["machine_gates_pass"] for system in verified]
    top = {
        ...
        "mixed_topology_reproduced": topology == [True, True, False]
        and recorded["topology"] == [True, True, False],
    ...
    result = {
        "format": "abi-capability-compiler-phase4-v22-b40-independent-verify-result/1",
        "status": "PASS_INDEPENDENTLY_VERIFIED_MIXED_B40_V22_TOPOLOGY_NO_MINIMUM"
        if verification_pass
        else "FAIL_V22_B40_INDEPENDENT_VERIFICATION",
        ...
        "claim_boundary": (
            "Independently verified mixed B40 v22 development topology only. B50 remains "
            "stable sufficient but not minimal. No matched baseline, final test, Phase 4, "
