"""No-artifact fixed-budget hybrid native-coefficient audit for layer 0."""
...
FORMAT = "abi-capability-compiler-phase3-fixed-budget-hybrid-coefficient-audit/1"
...
def execute(root: Path, protocol_path: Path, output: Path) -> dict:
    ...
    if (
        ...
        or protocol.get("status") != "PREREGISTERED_NO_ARTIFACT_FIXED_BUDGET_HYBRID_COEFFICIENT_AUDIT"
        or protocol.get("artifact_write") != "PROHIBITED"
        or protocol.get("final_test_access") != "PROHIBITED"
        or protocol.get("sweeps_authorized") is not False
    ):
        raise Phase3Error("fixed-budget hybrid governance changed")
    for name, expected in protocol["bindings"].items():
        path = Path(name) if Path(name).is_absolute() else root / name
        if not path.is_file() or sha256_file(path) != expected:
            raise Phase3Error(f"fixed-budget hybrid binding changed: {name}")
    if output.exists() or not torch.cuda.is_available():
        raise Phase3Error("fixed-budget hybrid output exists or CUDA unavailable")
    ...
    output_rank = int(protocol["output_rank"])
    basis = eigenvectors.flip(1)[:, :output_rank].contiguous()
    energy = float(eigenvalues[:output_rank].sum() / eigenvalues.sum().clamp_min(1e-12))
    ...
    target_coefficients = (residuals - mean) @ basis
    full_linear, full_ridge = closed.solve_ridge(
        features, target_coefficients, float(protocol["relative_ridge"])
    )
    left, singular_values, _ = torch.linalg.svd(full_linear, full_matrices=False)
    shared_rank = int(protocol["hybrid_map"]["shared_rank"])
    shared_projection = left[:, :shared_rank] * singular_values[:shared_rank]
    del left, singular_values, full_linear
    coordinates = features @ shared_projection
    hybrid_features = torch.cat((coordinates, F.silu(coordinates)), dim=-1)
    hybrid_outputs, hybrid_ridge = closed.solve_ridge(
        hybrid_features, target_coefficients, float(protocol["relative_ridge"])
    )
    linear_output, nonlinear_output = hybrid_outputs.chunk(2, dim=0)
    hybrid_train = coordinates @ linear_output + F.silu(coordinates) @ nonlinear_output
    coefficient_residual = target_coefficients - hybrid_train
    route_maps = []
    route_ridges = []
    route_observations = []
    for route in range(3):
        ...
        route_maps.append(route_map); route_ridges.append(route_ridge)
        route_observations.append(int(indices.numel()))
    mapped_train = hybrid_train.clone()
    for route in range(3):
        indices = torch.nonzero(routes == route, as_tuple=False).squeeze(1)
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        for row in validation_cache:
            ...
            coordinate = feature @ shared_projection
            coefficients = coordinate @ linear_output + F.silu(coordinate) @ nonlinear_output
            coefficients = coefficients + row["sparse"].to(device) @ route_maps[int(row["route"])]
    ...
    map_values = shared_projection.numel() + linear_output.numel() + nonlinear_output.numel()
    gates = {
        "rank_energy": energy >= float(protocol["gates"]["rank_energy_minimum"]),
        "parameter_budget_exact": map_values == int(protocol["hybrid_map"]["values"]),
    ...
    result = {
        "format": FORMAT,
        "status": "PASS_FIXED_BUDGET_HYBRID_COEFFICIENT_MAPPING" if passed else "FAIL_FIXED_BUDGET_HYBRID_COEFFICIENT_MAPPING",
        "protocol_sha256": sha256_file(protocol_path),
        "output_rank": output_rank, "rank_energy": energy,
        "train_observations": observations,
        "hybrid_map": {"shared_rank": shared_rank, "values": map_values},
        "full_linear_effective_ridge": full_ridge,
        "hybrid_effective_ridge": hybrid_ridge,
        ...
        "artifact_written": False, "final_test_accessed": False, "phase3_certified": False,
        "claim_boundary": "No-artifact fixed-budget hybrid coefficient capacity audit at layer 0 only; no extracted model, autonomous quality, Phase 3, or superiority claim.",
    }
...
def main() -> int:
    ...
    parser.add_argument("--protocol", default="ABI_CAPABILITY_COMPILER_PHASE3_FIXED_BUDGET_HYBRID_COEFFICIENT_AUDIT_PROTOCOL_V364.json")
    parser.add_argument("--output-dir", default="results/abi_capability_compiler_phase3_native_trajectory/hybrid_coefficient_audit_v365")
