...
def _mount_state() -> dict[str, Any]:
    ...
    development_mount_visible = any(
        token in text for token in (" /mnt/c ", " C:\\", "drvfs")
    ...
    *,
    ...
    completed = subprocess.run(
        [
            ...
            (
                "set -euo pipefail; "
                f"capsule='{linux_capsule}'; "
                f"cp -aL '{capsule_source}' \"$capsule\"; "
                "cat >\"$capsule/run-isolated.sh\" <<'SH'\n"
                "#!/bin/bash\n"
                "set -euo pipefail\n"
                "umount -l /mnt/c 2>/dev/null || true\n"
                "if mountpoint -q /mnt/c || test -e /mnt/c/Python310; then exit 91; fi\n"
                "cd \"$1/abi_release\"\n"
                f"python3 -m abi_v2.isolated_certification worker --capsule \"$1\" --host {host_key} --device {device}\n"
                "SH\n"
                "chmod 500 \"$capsule/run-isolated.sh\"; "
                "unshare --user --map-root-user --mount --propagation private "
                "\"$capsule/run-isolated.sh\" \"$capsule\"; "
                f"destination_path='{destination_target}'; "
                "mkdir -p \"$(dirname \"$destination_path\")\"; "
                "cp -a \"$capsule/abi_release/output\" \"$destination_path\"; "
                "printf '%s' \"$capsule\""
            ),
        ],
        cwd=root,
